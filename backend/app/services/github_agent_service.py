...
class GithubAgentService:
    ...
    async def execute_task_and_pr(
        ...
        feature_branch = branch_name or f"devmentor/agent-task"
        ...
        pr_result = await self._create_pr(
            owner=owner,
            repo=repo,
            title=f"[DevMentor Agent] {task[:80]}",
            body=(
                f"## 🤖 DevMentor AI Agent Task\n\n"
                f"**Task:** {task}\n\n"
                f"**File modified:** `{file_path}`\n\n"
                f"**Generated by:** DevMentor AI OS (Gemini 2.0 Flash)\n"
            ),
            head=feature_branch,
            base=default_branch,
        )
