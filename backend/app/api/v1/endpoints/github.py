...
@router.post('/sync')
async def sync_github(user_id: str = Depends(get_current_user_id), db: Session = Depends(get_db)):
    ...
    if not profile or not profile.access_token:
        raise HTTPException(status_code=400, detail="GitHub profile not connected or access token missing")

    try:
        github_service = GithubService(db)
        sync_result = await github_service.sync_user_github_data(user_id=user_id, access_token=profile.access_token)
        return {'success': True, 'message': 'GitHub sync completed', 'details': sync_result}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to sync GitHub data: {str(e)}")


@router.get('/profile')
def github_profile(user_id: str = Depends(get_current_user_id), db: Session = Depends(get_db)):
    stmt = select(GithubProfile).where(GithubProfile.user_id == user_id)
    profile = db.scalar(stmt)
    if not profile:
        ...
        if user and user.username:
            return {'login': user.username, 'name': user.name, 'avatar_url': user.avatar_url, 'synced_at': None}
...
def github_repositories(user_id: str = Depends(get_current_user_id), db: Session = Depends(get_db)):
...
def github_languages(user_id: str = Depends(get_current_user_id), db: Session = Depends(get_db)):
    stmt = select(Repository).where(Repository.user_id == user_id)
    repos = db.scalars(stmt).all()
    if not repos:
        return {'languages': {}}
...
async def get_github_contributions(username: str, access_token: str, year: int) -> list:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            import logging
            logging.getLogger(__name__).error(f"Error fetching GraphQL contributions: {e}")
    return []


@router.get('/activity')
async def github_activity(year: int | None = None, user_id: str = Depends(get_current_user_id), db: Session = Depends(get_db)):
    import random
    ...
    if not year:
        if profile and profile.access_token:
            try:
                ...
                real_contribs = await get_github_contributions(profile.login, profile.access_token, curr_year)
                if real_contribs:
        ...
        for i in range(70):
            ...
            activity_values.append({
                'date': date_str,
                'count': random.choice([0, 0, 1, 2, 4, 8, 10])
            })
    ...
    if profile and profile.access_token:
        try:
            real_contribs = await get_github_contributions(profile.login, profile.access_token, year)
            if real_contribs:
                return {
                    'year': year,
                    'is_real': True,
                    'activity': real_contribs
                }
        except Exception as e:
            import logging
            logging.getLogger(__name__).error(f"Failed to fetch contributions for year {year}: {e}")
    ...
    for i in range(days_in_year):
        ...
        activity_values.append({
            'date': date_str,
            'count': random.choice([0, 0, 0, 1, 2, 4, 8, 0])
        })
    return {
        'year': year,
        'is_real': False,
...
async def github_day_activity(date: str, user_id: str = Depends(get_current_user_id), db: Session = Depends(get_db)):
    from app.models.entities import GithubProfile
    stmt = select(GithubProfile).where(GithubProfile.user_id == user_id)
    profile = db.scalar(stmt)
    if not profile or not profile.access_token:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for item in collection.get("commitContributionsByRepository", []):
                    ...
                    if count > 0:
                        details.append(f"Committed {count} time{'' if count == 1 else 's'} to {repo_name}")
                ...
                for item in collection.get("pullRequestContributionsByRepository", []):
                    ...
                    count = item.get("contributions", {}).get("totalCount", 0)
                    if count > 0:
                        details.append(f"Opened {count} pull request{'' if count == 1 else 's'} in {repo_name}")
                ...
                for item in collection.get("issueContributionsByRepository", []):
                    ...
                    count = item.get("contributions", {}).get("totalCount", 0)
                    if count > 0:
                        details.append(f"Opened {count} issue{'' if count == 1 else 's'} in {repo_name}")
                ...
                for item in collection.get("pullRequestReviewContributionsByRepository", []):
                    ...
                    count = item.get("contributions", {}).get("totalCount", 0)
                    if count > 0:
                        details.append(f"Reviewed {count} pull request{'' if count == 1 else 's'} in {repo_name}")
                        
                summary = f"Activity on {date}:" if details else f"No contributions found on {date}."
                return {
                    'date': date,
                    'has_real_data': True,
                    'summary': summary,
